        ...
        while (transferSize > 0) {
            ...
            if (writeMode == DmaWriteMode.ONE_BYTE) {
                if (transferSize-- > 0)this.console.bus.writeByte(
                    ...
                    this.console.bus.readByte(Address.create(source)));
                count += 1;
            } else if (writeMode == DmaWriteMode.TWO_BYTES) {
                if (transferSize-- > 0) this.console.bus.writeByte(
                    Address.create(destination + 0),
                    this.console.bus.readByte(Address.create(source + 0)));
                if (transferSize-- > 0) this.console.bus.writeByte(
                    Address.create(destination + 0),
                    this.console.bus.readByte(Address.create(source + 1)));
                count += 2;
                ...
                if (transferSize-- > 0) this.console.bus.writeByte(
                    ...
                    this.console.bus.readByte(Address.create(source + 2)));
                if (transferSize-- > 0) this.console.bus.writeByte(
                    Address.create(destination + 3),
                    this.console.bus.readByte(Address.create(source + 3)));
                count += step * 4;
            } else {
